...
int main(int argc, char** argv) {
	...
	try {
		...
		ifaces.add_options()
			...
			("listen-addr",po::value<vector<string> >(&laddr)->multitoken(),
				"Specify local ip4 addresses to listen on. It is assumed that these ip"
				" addresses can be bound to by dewd." )
			("listen-port",po::value<int>()->default_value(2023),
				"Give a port number to listen on.  Used with --listen-addr.  It is"
				" assumed that dewd can bind to the given port.")
			("waltr-addr", po::value<vector<string> >(&waddr)->multitoken(),
				"Specify remote ip4 addresses to send commands to.  This option is"
				" written with the intent to issue network commands to a dewd.")
			("waltr-port", po::value<int>()->default_value(2023),
				"Give a remote port number.  Used with --waltr-addr.")
			;// We might want to change these to ip_address:port?
		...
		const int port_number = vmap["listen-port"].as<int>();
		...
		for(auto it : laddr)
			dis->make_ns(address_v4::from_string(it),port_number);
